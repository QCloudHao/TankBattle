...
TANK_RESOURCES_POS = './resources/'


# 加载图片
def load_img(name_img):
    save = TANK_RESOURCES_POS + name_img + '.gif'
    return pygame.image.load(save)


# 加载背景音乐
def load_music(name_music):
    save = TANK_RESOURCES_POS + name_music + '.wav'
...
class TankGame:
    ...
    def __init__(self):
        ...
        self.my_tank_lift = 3
        self.create_enemy(self.creat_enemy_number)
        self.create_walls()
    ...
    def start_game(self):
        ...
        while True:
            ...
            self.window.blit(self.draw_text('敌方坦克*{0},我方生命值*{1},当前{2}关'.format(len_enemy, self.my_tank_lift,
                                                                               self.number)), (10, 10))
            if len_enemy == 0:
                self.creat_enemy_number += 10
                self.number += 1
